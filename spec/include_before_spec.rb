...
describe RSpecSolr do
  ...
  context "should include().before()" do
    ...
    it "passes when criteria are met" do
      ...
      expect(@solr_resp_5_docs).to include("333").before("fld"=>["val1", "val2", "val3"])
      expect(@solr_resp_5_docs).to include("fld"=>"val").before("fld"=>["val1", "val2", "val3"])
      expect(@solr_resp_5_docs).to include(["111", "222"]).before(["333", "555"])
  ...
  context "should_NOT include().before()" do
    ...
    it "fails when criteria are met" do
      ...
      expect {
        expect(@solr_resp_5_docs).not_to include("333").before("fld"=>["val1", "val2", "val3"])
      }.to raise_error.with_message a_string_including 'not to include document "333" before document matching {"fld"=>["val1", "val2", "val3"]}'
      expect {
        expect(@solr_resp_5_docs).not_to include("fld"=>"val").before("fld"=>["val1", "val2", "val3"])
      ...
      expect {
        expect(@solr_resp_5_docs).not_to include(["111", "222"]).before(["333", "555"])
    ...
    it "passes when docs aren't in expected order" do
        ...
        expect(@solr_resp_5_docs).not_to include(["111", "444"]).before([{"id"=>"333"}, {"id"=>"555"}])
